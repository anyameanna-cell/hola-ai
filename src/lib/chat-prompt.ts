...
export function buildSystemPrompt(ctx: ChatContext): string {
  ...
  const lines: string[] = [
    ...
    "  ```choices",
    "  {\"question\": \"Which style do you want?\", \"type\": \"single\", \"options\": [\"Casual\", \"Formal\", \"Playful\"], \"allowOther\": true}",
    "  ```",
    "- `type` is \"single\" (pick one) or \"multiple\" (pick any that apply). Give 2-6 short options. Set `allowOther` to true when a free-text answer makes sense.",
    "- Put the block at the very end of your reply, after a short lead-in sentence. Never use more than one choices block per reply, and don't also ask the same question in plain text.",
